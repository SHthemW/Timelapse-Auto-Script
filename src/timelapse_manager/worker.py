...
def run_worker(task_id: str, root: Path | None = None, *, console: bool = False) -> int:
    ...
    try:
        ...
        runtime.log(
            f"任务工作进程启动，预设={runtime.task['preset']}，PID={runtime.state['runner_pid']}"
        )
        ...
        runtime.finish("completed", "任务已完成", 0)
        runtime.log("任务已正常完成")
        return 0
    except HardStopRequested:
        if runtime:
            runtime.finish("stopped", "任务已强制停止", 130)
            runtime.log("任务已强制停止")
        return 130
    except (TimelapseError, OSError, ValueError) as exc:
        if runtime:
            runtime.log(f"任务失败: {exc}")
            runtime.finish("failed", "任务失败", 1, str(exc))
        else:
        ...
        if runtime:
            ...
            runtime.finish("failed", "任务发生未处理异常", 1, str(exc))
        else:
            print(
                "任务工作进程发生未处理异常:\n" + traceback.format_exc(),
                file=sys.stderr,
